...
class ChatGPT:
    ...
    async def async_generate_text(self, ai_model: str, context: list, prompt: str, max_retries: int = 3):
        timeout = 30
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Логгирование ошибки или вывод сообщения для дебага
                self.logger.error(f"Попытка {attempt + 1}/{max_retries} не удалась: {e}")
                if attempt == max_retries - 1:
                    raise  # Если это последняя попытка, пробрасываем исключение

                # Вторая попытка через официальный клиент
                try:
                    response = await asyncio.wait_for(
                    ...
                    self.logger.error(f"Официальный клиент: попытка {attempt + 1}/{max_retries} не удалась: {e2}")
                    if attempt == max_retries - 1:
                        self.logger.error(f"Генерация текста не удалась. Причина - {e2}")

            # Опционально можно добавить задержку перед новой попыткой
